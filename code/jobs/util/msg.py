...
def jpeg2000(bn):
    messagebox.showerror(f'{D.appname} pdf2png.py',f'''
...
def finish():
    messagebox.showinfo(f'{D.appname} 正常終了',
    f'''処理がおわりました
...
def nw_err(e):
    messagebox.showerror(f'{D.appname} updn.py',(
        'OCRエンジンへの接続が拒否されました\n\n'
...
def cred_unset():
    ret = messagebox.askretrycancel(f'{D.appname} cred.py',(
...
def noimg(usrd):
    messagebox.showerror(f'{D.appname} mv2input.py',(
        '原稿のPDFが無いです\n\n'
...
def noxl(e):
    ...
    messagebox.showerror(f'{D.appname} loadxl.py',
                                        (f'''設定エクセルが開けません
